...
const Portfolio = () => {
    ...
    const onProjectSelectedHandler = (element) => {
        ReactGA.event({
    ...
    return (
        <Aux>
            <div id="portfolio" className={Style['portfolio']}>
                {!isLoading && projectList && Object.keys(projectList).map((key, i) => {
                    return <Project clicked={() => onProjectSelectedHandler(projectList[key])} project={projectList[key]} key={i} />
                })}
            </div>
            <ComingSoon />
        </Aux>
